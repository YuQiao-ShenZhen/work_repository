...
class Config:
    def __init__(self):
        ...
        self.sample_path = '../qts.txt'
        self.name = 'p13_exercise'
...
class SubTensors:
    def __init__(self, config, gpu_index, opt, ch_size):
        with tf.device('/gpu:%d' % gpu_index):
            ...
            cell1 = tf.nn.rnn_cell.BasicLSTMCell(config.num_units, state_is_tuple=True)
            cell2 = tf.nn.rnn_cell.BasicLSTMCell(config.num_units, state_is_tuple=True)
            ...
            with tf.variable_scope('rnn'):
                for i in range(config.num_step):
                    ...
                    loss_i = tf.nn.softmax_cross_entropy_with_logits(labels=yi, logits=yi_pred)
...
class Poem:
        ...
        for epoch in range(cfg.epoches):
            ...
            for batch in range(batches):
                ...
                _, loss, su, precise = self.session.run(
                    [self.tensors.train_op, self.tensors.loss, self.tensors.summary_op, self.tensors.precise],
                    feed_dict)
    ...
    def predict(self, head=None):
        ...
        for i in range(self.config.num_step-1):
            xi = [head[i]] if i < len(head) else yi_predict
            feed_dict = {
                self.tensors.sub_ts[0].xi: xi,
                self.tensors.sub_ts[0].input_state: state
            }
            yi_predict, state = self.session.run([self.tensors.sub_ts[0].yi_predict, self.tensors.sub_ts[0].state],
                                                 feed_dict)
            if i >= len(head)-1:
